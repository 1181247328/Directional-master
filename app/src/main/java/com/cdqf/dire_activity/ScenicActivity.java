...
import com.cdqf.dire_adapter.ScenicAdapter;
...
public class ScenicActivity extends BaseActivity {
    ...
    private String TAG = ScenicActivity.class.getSimpleName();
    ...
    private ScenicAdapter scenicAdapter = null;
    ...
    private void initAdapter() {
        scenicAdapter = new ScenicAdapter(context);
        lvCementPull.setAdapter(scenicAdapter);
    }
...
//                    if (aCache.getAsString("details2" + direState.getUser().getId() + direState.getScenicList().get(position).getId()) != null) {
//                        String gameJSON = aCache.getAsString("details2" + direState.getUser().getId() + direState.getScenicList().get(position).getId());
//                        game = gson.fromJson(gameJSON, DetailsGame.class);
//                        boolean isSelete = game.getAnswer().get(game.getAnswer().size() - 1).isSelete();
//                        if (isSelete) {
  //                          initIntentReward(RewardActivity.class, direState.getScenicList().get(position).getId());
//                        } else {
                            initIntent(DetailsGameActivity.class, position, 2);
//                        }
//                    }
                } else {
...
//                params.put("tatol", 10);
                ...
                okHttpRequestWrap.post(DirectAddaress.SCENERY, false, "请稍候", params, new OnHttpRequest() {
                    @Override
                    public void onOkHttpResponse(String response, int id) {
                        ...
                        if (isStatus) {
                            switch (error_code) {
                                //获取成功
                                case 0:
//                                    aCache.put("scenicpage", page);
//                                    page = 2;
                                    handler.sendEmptyMessage(0x002);
                                    pullToRefreshLayout.refreshFinish(PullToRefreshLayout.SUCCEED);
                                    direState.getScenicList().clear();
                                    List<Scenic> scenicList = gson.fromJson(data, new TypeToken<List<Scenic>>() {
                                    }.getType());
                                    direState.setScenicList(scenicList);
                                    if (scenicAdapter != null) {
                                        scenicAdapter.notifyDataSetChanged();
                                    }
                                    aCache.put("scenic", data);
                                    break;
                                case 20042:
                                    pullToRefreshLayout.refreshFinish(PullToRefreshLayout.SUCCEED);
    ...
    private void initBack() {
        ptrlCementPull.setPullDownEnable(true);
        ptrlCementPull.setPullUpEnable(false);
        if (aCache.getAsString("scenic") != null) {
            String data = aCache.getAsString("scenic");
//            page = (int) aCache.getAsObject("scenicpage");
            handler.sendEmptyMessage(0x002);
            direState.getScenicList().clear();
            List<Scenic> scenicList = gson.fromJson(data, new TypeToken<List<Scenic>>() {
            }.getType());
...
//        params.put("limit", page);
        //查询字样
        OKHttpRequestWrap okHttpRequestWrap = new OKHttpRequestWrap(context);
        okHttpRequestWrap.post(DirectAddaress.SCENERY, true, "请稍候", params, new OnHttpRequest() {
            @Override
            public void onOkHttpResponse(String response, int id) {
